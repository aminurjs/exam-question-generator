...
const ChemistryQuestionForm = ({ prev }) => {
  const navigate = useNavigate();
  const { form } = useSelector((state) => state.formSlice);
  const { chemistry } = form;
  console.log({ ...form });

  const [questions, setQuestions] = useState(chemistry || []);
  const addExam = () => {
  ...
  const handleSubmit = (e) => {
    ...
    const updateData = { chemistry: data };
  ...
  return (
    <form onSubmit={handleSubmit}>
      ...
      <div className="flex justify-end -mt-5 mb-5 mr-2">
        <button className="btn text-xl" type="button" onClick={addExam}>
          +
        </button>
      </div>
